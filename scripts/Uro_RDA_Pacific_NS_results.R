...
#### Interpolation = most common genotype across ALL individuals (Pacific & Pacific)
...
Pacific<-Genotypes[Genotypes$Pop == "WP" |
                      Genotypes$Pop == "CP" |
                      Genotypes$Pop == "RB" |
                      Genotypes$Pop == "HM" |
                      Genotypes$Pop == "TO",]
...
AllFreq <- aggregate(Pacific, by = list(Pacific$Pop), function(x) mean(x, na.rm = T)/2)
...
dbmem.all.ord <- ordistep(dbmem.mod0, scope = formula(dbmem.mod1),
                          direction = "both", permutations = 1000)

# NO SIGNIFICANT dbMEMs
...
screeplot(pca, type = "barplot", npcs=30, main="PCA Eigenvalues")
...
pRDAfull <- rda(AllFreq ~ PC1 + PC2 + annual_max + annual_mean + annual_min + annual_range,  Variables)
RsquareAdj(pRDAfull)

anova(pRDAfull)

pRDAclim <- rda(AllFreq ~ annual_max + annual_min + annual_mean + annual_range + Condition(PC1 + PC2),  Variables)
RsquareAdj(pRDAclim)
...
pRDAstruct <- rda(AllFreq ~ PC1 + PC2 + Condition(annual_max + annual_mean + annual_min + annual_range),  Variables)
...
RDA_env <- rda(AllFreq ~ annual_max + annual_min + annual_mean + annual_range + Condition(PC1 + PC2),  Variables)
...
######## STOPPING HERE AS NOTHING IS SIGNIFICANT AND MODELS ARE UNCONSTRAINED
